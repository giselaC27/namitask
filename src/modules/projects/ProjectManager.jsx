  ...
  const createNewProject = async () => {
    try {
      ...
      const newProject = {
        name: newProjectName,
        idOwner: userId,
        tasks: [], // Aquí se añaden las tareas del nuevo proyecto si las tienes disponibles
      };

      // Realiza la solicitud POST al servidor para crear el proyecto
      const response = await axios.post(
        "http://localhost:8080/api/v2/proyect",
        newProject
      );
      ...
      if (response.status === 200) {
        ...
        setProjects([...projects, response.data]); //  Usar el estado actualizado
        setNewProjectName("");
        setShowProjectForm(false);
      } else {
        // Si la respuesta es falsa, hubo un error al crear el proyecto
        alert(
          "Hubo un error al crear el proyecto. Por favor, inténtelo de nuevo."
        );
      }
    } catch (error) {
...
// manejar la carga inicial de los proyectos del usuario y actualizar la interfaz
  useEffect(() => {
    const userId = user.id;
    axios
      .get(`http://localhost:8080/api/v2/proyects/${userId}`)
      .then((response) => {
  ...
  }, [user.id, projects]); // Agregar user.id a la lista de dependencias
...
//unirse a un proyecto
  ...
  const handleJoinProjectSubmit = async () => {
    try {
      //se extrae el valor de invitationCode
      const codeInvitationValue = invitationCode;

      const response = await axios.put(
        `http://localhost:8080/api/v2/proyect/${codeInvitationValue}/${user.id}`
      );
      ...
      if (response.status === 200) {
        ...
        console.error(
          "Error al unirse al proyecto: Código de invitación incorrecto"
        );
      } else {
  ...
  const handleDeleteProject = async (projectId, isShared) => {
    if (isShared) {
      alert("No puedes eliminar proyectos compartidos.");
      return;
    }
  
    const shouldDelete = window.confirm("¿Estás seguro que deseas eliminar este proyecto? ");
    ...
    try {
      const response = await axios.delete(`http://localhost:8080/api/v2/proyect/${projectId}`);
      if (response.status === 200) {
  ...
  return (
    <div className="w-1/4 bg-gray-200 p-4">
      ...
      <ProjectList projects={ownProjects} onSelect={handleSelectProject}   onDelete={handleDeleteProject} />
